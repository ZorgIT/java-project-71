...
final class Differ {
    ...
    public static void generate(final String json1,
                                final String json2) throws Exception {
    ...
    public static List<List<String>> checkData(final Map<String, Object> oldMap,
        ...
        for (Map.Entry<String, Object> entryOld : oldMap.entrySet()) {
            ...
            if (newValue != null) {
                //в2 Такой ключ есть, доп проверка
                // - ключ имеет одинаковые значения
                if (newValue.equals(oldValue)) {
                    lineStatus.add(lineAdd(" ", curKey, oldValue));
                } else {
                    //  в3 Такой ключ есть, доп проверка
                    //  - ключи имеют разные значения = установить
                    lineStatus.add(lineAdd("-", curKey, oldValue));
